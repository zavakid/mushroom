...
import com.zavakid.mushroom.impl.ConfigBuilder;
...
public class TestPatternFilter {
    ...
    public void includeOnlyShouldOnlyIncludeMatched() {
        ...
        shouldAccept(wl, "foo");
        shouldAccept(wl, Arrays.asList(new MetricsTag("bar", "", ""), new MetricsTag("foo", "", "f")));
        shouldReject(wl, "bar");
    ...
    public void excludeOnlyShouldOnlyExcludeMatched() {
        ...
        shouldReject(bl, "foo");
        shouldReject(bl, Arrays.asList(new MetricsTag("bar", "", ""), new MetricsTag("foo", "", "f")));
    }

    /**
     * Filters should accepts unmatched item when both include and exclude patterns are present.
     */
    @Test
    public void shouldAcceptUnmatchedWhenBothAreConfigured() {
        SubsetConfiguration c = new ConfigBuilder().add("p.include", "foo").add("p.include.tags", "foo:f").add("p.exclude",
                                                                                                               "bar").add("p.exclude.tags",
                                                                                                                          "bar:b").subset("p");
        shouldAccept(c, "foo");
